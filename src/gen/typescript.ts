...
import { PulseQuantizedTypes, PulseType } from "../types";
import {
...
const TO_QUNATIZER_WRITE_TYPE: Record<
  PulseType.F32,
  Record<PulseType.Q8 | PulseType.Q16, string>
> = {
  [PulseType.F32]: {
...
export class TypeScriptGenerator {
    ...
    for (const field of pkg.fields) {
      if (field.type.externalType) {
        if (field.type.quantizedStep) {
          if (
            PulseQuantizedTypes.includes(field.type.internalType) &&
  ...
  private generateQuantizeFieldWriter(field: ParserField) {
    const qunatizer =
      TO_QUNATIZER_WRITE_TYPE[field.type.internalType as PulseType.F32][
        field.type.externalType! as PulseType.Q8
      ];
    this.writeLine(
      ...
      if (field.type.externalType) {
        if (field.type.quantizedStep) {
          if (
            ...
            this.generateQunatizeFieldReader(field);
            continue;
          }
        }
        this.generateExternalFieldReader(field);
        continue;
      }
      this.generateFieldReader(field);
    }
    this.writeLines("\t\t}", "\t}");
  }

  private generateQunatizeFieldReader(field: ParserField) {
    const qunatizer =
  ...
  private generateExternalFieldReader(field: ParserField) {
    this.writeLines(
      `\t\t\t${field.name}: reader.${TO_READER_FUNCTION[field.type.externalType!]}(),`,
    );
  }

  private generateFieldReader(field: ParserField) {
    this.writeLines(
