...
export class GenerateRustWrite {
  ...
  private static genField(typescript: RustGenerator, field: ParserField) {
    if (field.isPartial && !field.isStatic) {
      if (field.type.isArray) {
        typescript.writeLines(
          `\t\tif let Some(${field.name}) = &value.${field.name} {`,
          `\t\t\twriter.write_var_u32(${field.name}.len() as u32);`,
          `\t\t\tfor i in ${field.name}.unwrap() {`,
          `\t\t\t\t${this.genValue(field, field.type.internalType !== PulseType.NESTED && field.type.internalType !== PulseType.STR, "i", true)}`,
          `\t\t\t}`,
          `\t\t}`,
        );
      } else {
        typescript.writeLines(
          `\t\tif let Some(${field.name}) = &value.${field.name} {`,
          `\t\t\t${this.genValue(field, field.type.internalType !== PulseType.NESTED && field.type.internalType !== PulseType.STR, undefined, true)}`,
          `\t\t}`,
        );
      }
    } else {
      if (field.type.isArray) {
        typescript.writeLines(
          ...
          `\t\t\t\t${this.genValue(field, field.type.internalType !== PulseType.NESTED && field.type.internalType !== PulseType.STR, "i")}`,
          `\t\t\t}`,
        );
      } else {
  ...
  private static genValue(
    ...
    if (field.type.isNestedType) {
      return `${field.type.isNestedType}::write_package(${
        arrayIndex
          ? `${arrayIndex}`
          : `${unref ? "*" : ""}${asPrimitive ? "" : "value."}${field.name}`
      }, writer);`;
      ...
      if (field.type.externalType)
        return `writer.${
          TO_WRITER_RUST_FUNCTION[field.type.externalType]
        }(Quantizer::${quantizer}(
        ${
          arrayIndex
            ? `*${arrayIndex}`
            : `${unref ? "*" : ""}${asPrimitive ? "" : "value."}${field.name}`
        }, ${field.type.quantizedStep}));`;
      else
        return `writer.${TO_WRITER_RUST_FUNCTION[field.type.internalType]}
      (Quantizer::${quantizer}(${
        arrayIndex
          ? `*${arrayIndex}`
          : `${unref ? "*" : ""}${asPrimitive ? "" : "value."}${field.name}`
      ...
      if (field.type.externalType)
        return `writer.${TO_WRITER_RUST_FUNCTION[field.type.externalType]}(${
          arrayIndex
            ? `*${arrayIndex}`
            : `${unref ? "*" : ""}${asPrimitive ? "" : "value."}${field.name}`
        } as ${TO_RUST_TYPE[field.type.externalType]});`;
      else if (field.type.internalType === PulseType.STR)
        return `writer.${TO_WRITER_RUST_FUNCTION[field.type.internalType]}(${
          arrayIndex
            ? `*${arrayIndex}`
            : `${unref ? "*" : ""}${asPrimitive ? "" : "value."}${field.name}.clone()`
        });`;
      else
        return `writer.${TO_WRITER_RUST_FUNCTION[field.type.internalType]}(${
          arrayIndex
